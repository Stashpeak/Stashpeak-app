...
export function ProductNode({ data }: NodeProps<ProductGraphNode>) {
  ...
  return (
    <div className={`relative map-node-tone ${data.tone.className}`}>
      <Handle id="provider" type="target" position={Position.Top} className="map-node-handle-hidden" />
      <Handle id="subscription" type="target" position={Position.Bottom} className="map-node-handle-hidden" />
      <div
