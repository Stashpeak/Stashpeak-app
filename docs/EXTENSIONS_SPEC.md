...
**KB write broker (storage broker — planned first realization):** the KB MCP write broker (`MCP_KB_CONTRACT.md` §8) will be the first concrete realization of the long-deferred `file_write` storage broker sketched in `effects.file_write` above; it is **planned as core-owned** (like the `SYNC_ENGINE.md` §6.7 write path), not effects-gated, because the KB connector is a built-in core extension that will ride the same trusted-native path as the sync engine rather than being dispatched through the extension sandbox. This PR covers the read-only foundation; the write broker is a future deliverable.
...
- [x] Capability manifests name **network hosts** + read/write scopes (not just budgets/booleans). *(network hosts populated + enforced in #122; storage scopes await a general storage broker — the KB write broker in `MCP_KB_CONTRACT.md` §8 is planned as the first realization of `file_write`, to be core-owned and not effects-gated; see §7. This PR is read-only; the write broker is a future deliverable.)*
