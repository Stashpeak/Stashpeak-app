...
**Host-brokered fetch (the key change):** the trait gains a capability context — `fetch(&self, ctx: &ConnectorCtx)`. `ctx.credential()` returns **only this connector's** secret, keyed by the descriptor `id` (identity-bound, not a string the connector chooses). The host injects the credential / performs egress; **the raw key never enters the connector body**. `secrets::get_provider_api_key` becomes crate-private; all egress flows through `connectors::http` as the sole broker, checked against `permissions.network` (advisory-logged in v1, enforced in v2).
...
**Honest enforcement framing:** in v1, `effects` is enforced *by the broker* (a handler has no other reach) + declaration; a hard sandbox arrives with v2/WASM. The descriptor schema (esp. `effects` naming network **hosts** + read/write scopes) must be complete **now** — it is the one thing that cannot change without breaking every extension.
